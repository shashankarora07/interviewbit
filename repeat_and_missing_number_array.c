...
{
	long long int sum = 0, sum_sq = 0, diff1 = 0, diff2 = 0, diff = 0;
	int i = 0, repeat = 0, miss = 0;
	...
	//sum = (len * (len+1) / 2) + repeat - miss;
	//diff1 <--= miss - repeat = (len * (len+1) / 2) - sum;
	diff1 = (len * (len+1) / 2) - sum;
	
	//sum_sq = (len * (len+1) * (2*n+1) / 6) + repeat * repeat - miss * miss;
	//diff2 <--= (miss * miss - repeat * repeat) = (len * (len+1) * (2*n+1) / 6) - sum_sq;
	diff2 = (len * (len+1) * (2*len+1) / 6) - sum_sq;
	//(miss - repeat)(miss + repeat)/ (miss - repeat) = ((len * (len+1) * (2*n+1) / 6) - sum_sq) / (len * (len+1) / 2) - sum;
	//miss + repeat = ((len * (len+1) * (2*n+1) / 6) - sum_sq) / (len * (len+1) / 2) - sum;
	diff = diff2 / diff1;
	//Note: means solve these two equations : 
	//diff <--=  miss + repeat = ((len * (len+1) * (2*n+1) / 6) - sum_sq) / (len * (len+1) / 2) - sum;
	//diff1 <--= miss - repeat = (len * (len+1) / 2) - sum; (multiply this equation by -1 and then find value of repeat)
	repeat = (diff - diff1) / 2;
	miss = (diff + diff1) / 2;
	out[0] = repeat;
	out[1] = miss;
